...
from user.tests.aarp_pyp_join_page_with_parameter import PypJoinPage
from user.Config.config import Global_Env_Data
import csv


class TestAarpPypJoinPageWithParameter(BaseTemplate):
    def test_join(self):
        self.contents = []
        path_to_file = (
    ...
    def perform_test_steps_premium1(self):
        ...
        self._qs_json_application_page = self.join_page.qs_as_json_object(
            self._application_page_url
        )

        self.join_page.compare_all_qs_params_in_json_for_pyp(
